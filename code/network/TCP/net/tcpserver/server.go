...
func (s *Server) Start(factory IoHandlerFactory) error {

	s.ioHandlerFactory = factory
	ClientConnMap = make(map[string]net.Conn)

	errorChannel := make(chan error)
	defer close(errorChannel)
	go s.startTCP(errorChannel)

	err := <-errorChannel
	if err == nil {
		s.stopListenChannel = make(chan bool)
	} else {
...
func (s *Server) onConn(conn net.Conn) {
	...
	ioConn.Start(s.ioHandlerFactory.CreateIoHandler(), conn, 10000)
}
